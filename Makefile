...
submission:
	mkdir -p _submission
	tar --exclude='./_submission' --exclude='./.git' --exclude='./_test' --exclude='./doc' --exclude='./github' --exclude='./doc' --exclude='./.gitignore' -czf _submission/submission.tgz .
...
test-script:
	...
	python3 src/test_script_task3.py
	python3 src/test_script_task4.py
